...
describe("Modal", () => {
  ...
  it("can be instantiated with title and subtitle", function () {
    ...
    expect(modal.select(".ark-modal__title").textContent.trim()).toEqual(
      "MyTitle"
    )

    expect(modal.select(".ark-modal__subtitle").textContent.trim()).toEqual(
      "MySubtitle"
    )
  })
  ...
  it("can be rendered with slots", function () {
    ...
    expect(modal.select(".ark-modal__body").textContent.trim()).toEqual(
      "MyBody"
    )

    expect(
      modal
        .select(".ark-modal__actions")
        .firstElementChild.hasAttribute("close")
    ).toBeTruthy()
  })
  ...
  it("closes on child data modal close", () => {
    container.innerHTML = /* html */ `
      <ark-modal title="myTitle">
        <div>MyBody</div>
        <div slot="action" close>close</div>
      </ark-modal>
    `
    const modal = container.querySelector("ark-modal")

    const closeDiv = modal.select("[close]")
