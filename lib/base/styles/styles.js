...
const effects = ['filter', 'mix-blend-mode', 'transition', 'transform']
...
${['display', ...effects].map(effect => actions.map(action => css`
[style*='--child-${effect}${action}:']${
  action.replace('-', ':')} > *:last-child {
  ${effect}: var(--child-${effect}${action});
}
`.trim())).flat().join('\n')}
